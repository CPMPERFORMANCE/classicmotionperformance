const t = {
  lv: {
    // Navbar
    nav_search: 'Meklēt...',
    nav_contact_btn: 'Sazināties',
    nav_back: 'Atpakaļ',
    nav_menu: 'Izvēlne',
    nav_specialists: 'Mk2 Speciālisti',
    nav_home: 'Sākums',
    nav_products: 'Produkti',
    nav_gallery: 'Galerija',
    nav_about: 'Par mums',
    nav_contacts: 'Kontakti',

    // Footer
    footer_desc: 'Premium bremžu un balstiekārtas uzlabojumi veiktspējai orientētiem projektiem.',
    footer_nav: 'Navigācija',
    footer_contact: 'Kontakti',
    footer_rights: 'Visas tiesības aizsargātas.',
    footer_tagline: 'Radīts Leģendām',

    // Home
    home_eyebrow: 'PREMIUM VEIKTSPĒJAS UZLABOJUMI',
    home_cmp_q: 'Kas ir',
    home_cmp_name: 'Classic Motion Performance',
    home_cmp_desc: 'Classic Motion Performance ir Latvijā radīts performance zīmols, kas apvieno klasisko auto kultūru ar modernu pieeju. Mēs fokusējamies uz Mk2 platformu, radot projektus entuziastiem, kuri novērtē kvalitāti, detaļas un īstu VAG kultūru.',
    home_read_more: 'Lasīt vairāk',
    home_new_in_store: 'Jaunumi',
    home_view_all: 'Skatīt visus',
    home_most_popular: 'Populārākie',
    home_top_picks: 'Top Izvēle',
    home_cta_eyebrow: 'Sāciet šodien',
    home_cta_title: 'Gatavs uzlabot savu projektu?',
    home_cta_desc: 'Aplūkojiet mūsu klāstu vai sazinieties ar mums — mēs atbildēsim personīgi.',
    home_explore: 'Apskatīt produktus',
    home_get_in_touch: 'Sazināties',
    home_hero_mobile_title: 'Bremžu &\nbalstiekārtas uzlabojumi',
    home_hero_mobile_sub: 'VW Golf Mk2 · Latvija',
    home_ticker: ['Saglabā klasiku', 'Klasiskas VW Performance detaļas', 'Radīts MK2 modeļiem', 'Klasiska kustība', 'Mūsdienīga kvalitāte', 'Īstiem projektiem radītas detaļas', 'Radīts 1983–1992 ikoniskiem auto', 'Dzimis no 6,3 miljoniem leģendu'],

    // Gallery page
    gallery_eyebrow: 'MŪSU DARBI',
    gallery_title: 'Galerija',
    gallery_empty: 'Galerija tiek papildināta',

    // Partners
    partners_eyebrow: 'MŪSU DRAUGI',

    // Products page
    products_eyebrow: 'Mūsu Klāsts',
    products_title: 'Produkti',
    products_desc: 'Mērķtiecīgi uzlabojumu risinājumi Volkswagen Golf Mk2 entuziastiem.',
    products_all: 'Visi',
    products_price_on_request: 'Cena pēc piepras.',
    products_not_found: 'Produkti nav atrasti',
    products_not_found_sub: 'Mēģiniet citu meklēšanas vaicājumu.',
    products_filter_category: 'Filtrēt pēc kategorijas',

    // How it works
    how_eyebrow: 'Kā Tas Darbojas',
    how_title: 'Vienkārši. Skaidri. Pārliecinoši.',
    how_steps: [
      { step: '01', title: 'Izvēlies savu risinājumu', text: 'Pārlūko mūsu produktus un izvēlies jaunināšanas virzienu, kas atbilst tava projekta mērķiem.' },
      { step: '02', title: 'Apstiprina piemērotību', text: 'Mēs apstiprinām saderību un piemērošanas detaļas tieši tavam Mk2 variantam pirms jebkā tiek uzsākts.' },
      { step: '03', title: 'Jaunini ar pārliecību', text: 'Saņem savu jauninājumu ar skaidriem uzstādīšanas norādījumiem un mērķtiecīga risinājuma garantiju.' },
    ],
    how_cta: 'Sākt Sarunu',

    // Product Detail
    detail_contact_title: 'Sazināties',
    detail_contact_sub: 'Aizpildi formu un mēs ar tevi sazināsimies.',
    detail_name: 'Vārds',
    detail_phone: 'Tālrunis',
    detail_sending: 'Nosūta...',
    detail_order: 'Pasūtīt',
    detail_thanks: 'Paldies!',
    detail_thanks_sub: 'Mēs ar tevi sazināsimies drīzumā.',
    detail_error: 'Kļūda. Lūdzu mēģini vēlreiz.',
    detail_description: 'Apraksts',
    detail_specs: 'Specifikācijas',
    detail_variants: 'Varianti',
    detail_not_found: 'Produkts nav atrasts',
    detail_back: 'Atpakaļ',

    // About page
    about_eyebrow: 'Kas Mēs Esam',
    about_title: 'Classic Motion',
    about_subtitle: 'SAVE THE CLASSIC',
    about_story_title: 'CMP Stāsts',
    about_story_p1: 'CMP Classic Motion Performance radās no aizrautības pret klasiskajiem Volkswagen automobiļiem un īstu autosporta kultūru. Mūsu mērķis ir saglabāt un pilnveidot leģendārās 1983–1992 Volkswagen platformas, apvienojot OEM kvalitāti ar modernu Performance.',
    about_story_p2: 'Mēs specializējamies bremžu sistēmu uzlabojumos, piekares risinājumos un custom performance detaļās, kas paredzētas gan ielas auto, gan entuziastu projektiem. Katrs projekts tiek veidots ar uzmanību pret detaļām, funkcionalitāti un klasisko Volkswagen raksturu.',
    about_story_p3: 'CMP nav tikai detaļas vai serviss — tā ir kustība cilvēkiem, kuri novērtē īstu auto kultūru, mehānisku sajūtu un klasisko modeļu identitāti.',
    about_story_tagline: 'SAVE THE CLASSICS.',
    about_focus: 'Fokuss',
    about_spec: 'Specializācija',
    about_philosophy: 'Filozofija',
    about_approach: 'Pieeja',
    about_spec_val: 'OEM+ & Performance MK2 Projekti',
    about_phil_val: 'Mūsdienīga Performance',
    about_approach_val: 'Radīts entuziastiem',
    about_values_eyebrow: 'Kas Mūs Virza',
    about_values_title: 'Mūsu Vērtības',
    about_val_1_title: 'Precizitāte',
    about_val_1_text: 'Katrs risinājums tiek izstrādāts ar augstāko uzmanību detaļām — gan tehniski, gan vizuāli.',
    about_val_2_title: 'Godīgums',
    about_val_2_text: 'Mēs iesakām tikai to, kas patiešām nepieciešams jūsu projektam. Skaidri, profesionāli un bez liekiem solījumiem.',
    about_val_3_title: 'Kvalitāte',
    about_val_3_text: 'Izmantojam pārbaudītus materiālus un komponentes, kuriem uzticamies paši.',
    about_val_4_title: 'Aizrautība',
    about_val_4_text: 'Mums tas nav tikai darbs — tā ir kultūra, ko veidojam ar attieksmi, pieredzi un rūpību.',
    about_quote: 'SAVE THE',
    about_quote_2: 'CLASSIC.',
    about_philosophy_label: '— CMP',
    about_cta_title: 'Gatavs runāt par savu projektu?',
    about_cta_desc: 'Mēs personīgi strādājam ar katru pieprasījumu. Pastāstiet mums, ko veidojat, un mēs sniedzam godīgu atbildi.',
    about_get_in_touch: 'Sazināties',
    about_view_products: 'Apskatīt produktus',

    // Contact page
    contact_eyebrow: 'Sazināsimies',
    contact_title: 'Sazinieties ar mums',
    contact_subtitle: 'Pastāstiet mums par savu projektu un meklēto uzlabojumu.',
    contact_name: 'Vārds',
    contact_email: 'E-pasts',
    contact_car: 'Auto / Projekts',
    contact_car_placeholder: 'piem. Golf Mk2 GTI 16V',
    contact_message: 'Ziņojums',
    contact_message_placeholder: 'Pastāstiet par savu projektu un ko vēlaties sasniegt...',
    contact_send: 'Nosūtīt',
    contact_sent_title: 'Paldies par ziņu',
    contact_sent_desc: 'Drīz sazināsimies.',
    contact_reply_title: 'Personīga atbilde, ne šablons.',
    contact_reply_desc: 'Mēs atbildam ar personīgu un pārdomātu pieeju, balstoties uz jūsu projektu un mērķiem. Katrs pieprasījums tiek uzmanīgi izlasīts.',
    contact_response_eyebrow: 'Atbildes laiks',
    contact_response_title: 'Personīgi. Pārdomāti. Godīgi.',
    contact_response_desc: 'Mēs neizmantojam automatizētas atbildes. Katra atbilde ir pielāgota jūsu konkrētajam projektam un mērķiem.',
    contact_email_label: 'E-pasts',
    contact_phone_label: 'Tālrunis',
    contact_ig_label: 'TikTok',
  },

  en: {
    // Navbar
    nav_search: 'Search...',
    nav_contact_btn: 'Get in Touch',
    nav_back: 'Back',
    nav_menu: 'Menu',
    nav_specialists: 'Mk2 Specialists',
    nav_home: 'Home',
    nav_products: 'Products',
    nav_gallery: 'Gallery',
    nav_about: 'About Us',
    nav_contacts: 'Contact',

    // Footer
    footer_desc: 'Premium brake and suspension upgrades for performance-focused builds.',
    footer_nav: 'Navigation',
    footer_contact: 'Contact',
    footer_rights: 'All rights reserved.',
    footer_tagline: 'Built for Legends',

    // Home
    home_eyebrow: 'PREMIUM PERFORMANCE UPGRADES',
    home_cmp_q: 'What is',
    home_cmp_name: 'Classic Motion Performance',
    home_cmp_desc: 'Classic Motion Performance is a Latvia-born performance brand that blends classic car culture with a modern approach. We focus on the Mk2 platform, building projects for enthusiasts who appreciate quality, detail, and true VAG culture.',
    home_read_more: 'Read more',
    home_new_in_store: 'New in Store',
    home_view_all: 'View all',
    home_most_popular: 'Most Popular',
    home_top_picks: 'Top Picks',
    home_cta_eyebrow: 'Get Started',
    home_cta_title: 'Ready to upgrade your build?',
    home_cta_desc: 'Browse our range or get in touch — we reply personally.',
    home_explore: 'Explore Products',
    home_get_in_touch: 'Get in Touch',
    home_hero_mobile_title: 'Brake &\nSuspension Upgrades',
    home_hero_mobile_sub: 'VW Golf Mk2 · Latvia',
    home_ticker: ['Save the Classic', 'Classic VW Performance Parts', 'Built for MK2', 'Classic Motion', 'Modern Performance', 'Performance Parts Made for Real Builds', 'Made for the Icons of 1983-1992', 'Born from 6.3 Million Legends'],

    // Gallery page
    gallery_eyebrow: 'OUR WORK',
    gallery_title: 'Gallery',
    gallery_empty: 'Gallery coming soon',

    // Partners
    partners_eyebrow: 'OUR FRIENDS',

    // Products page
    products_eyebrow: 'Our Range',
    products_title: 'Products',
    products_desc: 'Purpose-built upgrade solutions for Volkswagen Golf Mk2 enthusiasts.',
    products_all: 'All',
    products_price_on_request: 'Price on request',
    products_not_found: 'No products found',
    products_not_found_sub: 'Try a different search query.',
    products_filter_category: 'Filter by category',

    // How it works
    how_eyebrow: 'How It Works',
    how_title: 'Simple. Clear. Confident.',
    how_steps: [
      { step: '01', title: 'Choose your setup', text: 'Browse our products and identify the upgrade direction that fits your build goals.' },
      { step: '02', title: 'Confirm fitment', text: 'We confirm compatibility and fitment specifics for your exact Mk2 variant before anything moves forward.' },
      { step: '03', title: 'Upgrade with confidence', text: 'Receive your upgrade with clear installation guidance and the assurance of a purpose-built solution.' },
    ],
    how_cta: 'Start the Conversation',

    // Product Detail
    detail_contact_title: 'Get in Touch',
    detail_contact_sub: 'Fill in the form and we will get back to you.',
    detail_name: 'Name',
    detail_phone: 'Phone',
    detail_sending: 'Sending...',
    detail_order: 'Enquire',
    detail_thanks: 'Thank you!',
    detail_thanks_sub: 'We will be in touch shortly.',
    detail_error: 'Error. Please try again.',
    detail_description: 'Description',
    detail_specs: 'Specifications',
    detail_variants: 'Variants',
    detail_not_found: 'Product not found',
    detail_back: 'Back',

    // About page
    about_eyebrow: 'Who We Are',
    about_title: 'Classic Motion',
    about_subtitle: 'SAVE THE CLASSIC',
    about_story_title: 'The CMP Story',
    about_story_p1: 'CMP Classic Motion Performance was born from a passion for classic Volkswagen cars and true motorsport culture. Our goal is to preserve and enhance the legendary 1983–1992 Volkswagen platforms, combining OEM quality with modern performance.',
    about_story_p2: 'We specialise in brake system upgrades, suspension solutions, and custom performance parts designed for both street cars and enthusiast builds. Every project is built with attention to detail, functionality, and the classic Volkswagen character.',
    about_story_p3: 'CMP is not just parts or service — it is a movement for people who appreciate real car culture, mechanical feel, and the identity of classic models.',
    about_story_tagline: 'SAVE THE CLASSICS.',
    about_focus: 'Focus',
    about_spec: 'Specialisation',
    about_philosophy: 'Philosophy',
    about_approach: 'Approach',
    about_spec_val: 'OEM+ & Performance MK2 Projects',
    about_phil_val: 'Modern Performance',
    about_approach_val: 'Built for enthusiasts',
    about_values_eyebrow: 'What Drives Us',
    about_values_title: 'Our Values',
    about_val_1_title: 'Precision',
    about_val_1_text: 'We do not guess. Every solution is measured against the demands of the Mk2 platform — technically and visually.',
    about_val_2_title: 'Integrity',
    about_val_2_text: 'We recommend what is right for your build. Nothing more, nothing less. Honest advice, no overselling.',
    about_val_3_title: 'Quality',
    about_val_3_text: 'Only materials and components that meet a standard we are personally confident in. Full stop.',
    about_val_4_title: 'Passion',
    about_val_4_text: 'The Mk2 is not just a product category for us. It is a culture we are part of and genuinely care about.',
    about_quote: 'SAVE THE',
    about_quote_2: 'CLASSIC.',
    about_philosophy_label: '— CMP',
    about_cta_title: 'Ready to talk about your build?',
    about_cta_desc: 'We work personally with every enquiry. Tell us what you are building and we will give you an honest answer.',
    about_get_in_touch: 'Get in Touch',
    about_view_products: 'View Products',

    // Contact page
    contact_eyebrow: "Let's Talk",
    contact_title: 'Get in Touch',
    contact_subtitle: 'Tell us about your build and the upgrade you are looking for.',
    contact_name: 'Name',
    contact_email: 'Email',
    contact_car: 'Car / Build',
    contact_car_placeholder: 'e.g. Golf Mk2 GTI 16V',
    contact_message: 'Message',
    contact_message_placeholder: 'Tell us about your project and what you\'re looking to achieve...',
    contact_send: 'Send Message',
    contact_sent_title: 'Message Sent',
    contact_sent_desc: 'We reply with a more personal and considered approach, based on your build and goals. You will hear from us shortly.',
    contact_reply_title: 'A personal reply, not a template.',
    contact_reply_desc: 'We reply with a more personal and considered approach, based on your build and goals. Every enquiry is read carefully.',
    contact_response_eyebrow: 'Response time',
    contact_response_title: 'Personal. Considered. Honest.',
    contact_response_desc: 'We don\'t use automated responses. Every reply is tailored to your specific build and goals.',
    contact_email_label: 'Email',
    contact_phone_label: 'Phone',
    contact_ig_label: 'TikTok',
  },
};

export default t;
